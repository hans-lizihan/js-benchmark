...
const Row = (props) => {
  const { d } = props;
  return (
    <tr class={{'danger': props.selected}}>
      <td sel=".col-md-1">{d.id}</td>
      <td sel=".col-md-4"><a on={{click: () => props.onSelect(d.id)}}>{d.label}</a></td>
      <td sel=".col-md-1"><a on={{click: () => props.onRemove(d.id)}}><span sel=".glyphicon.glyphicon-remove" aria-hidden="true"></span></a></td>
      <td sel=".col-md-6"></td>
    </tr>
  );
}
            ...
            <table sel=".table.table-hover.table-striped.test-data">
              <tbody>
                {this.state.data.map(d => (
                  <Row key={d.id} selected={this.state.selected === d.id} d={d} onSelect={() => this.helpers.onSelect(d.id)} onRemove={() => this.helpers.onRemove(d.id)} />
                ))}
              </tbody>
            </table>
